...
namespace Banco_Academia
{
    public partial class F_Login : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sql = "select * from TB_User where UserName='" + username + "' and UserSenha='" + senha + "'";
            dt = Banco.dql(sql);

            if (dt.Rows.Count == 1)
            {
                form1.label3.Text = dt.Rows[0].ItemArray[5].ToString();
                form1.label4.Text = dt.Rows[0].Field<string>("UserName");
                form1.pictureBox1.Image = Properties.Resources.Green;

                Globais.nivel = int.Parse(dt.Rows[0].Field<Int64>("UserNivel").ToString());
                Globais.logado = true;
